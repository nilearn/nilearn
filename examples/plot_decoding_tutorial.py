...
# We will visualize the previously fetched :term:`fmri` data from Haxby dataset.
#
# Because :term:`fmri` data are 4D (they consist of many 3D EPI images), we cannot
...
# extract a single 3D EPI image from the :term:`fmri` data.
#
...
# has an interest to us for decoding, so we will keep only :term:`fmri` signals
# corresponding to faces or cats. We create a mask of the samples belonging to
# the condition; this mask is then applied to the :term:`fmri` data to restrict the
# classification to the face vs cat discrimination.
#
# The input data will become much smaller (i.e. :term:`fmri` signal is shorter):
