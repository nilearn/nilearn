...
for index, (contrast_id, contrast_val) in enumerate(contrasts.items()):
    # contrast_val = contrast_val[np.newaxis]
    output_type = 'z_score'
    z_map = fmri_glm.compute_contrast(
        contrast_val, output_type=output_type, stat_type='F')
    # 
    display = plotting.plot_stat_map(
        z_map, bg_img=mean_img_, threshold=2.5, title=contrast_id, vmax=10)
    display.savefig(path.join(write_dir, '%s_z_map.png' % contrast_id))
    #
    z_map_safe = fmri_glm.compute_contrast(
        contrast_val, output_type=output_type, stat_type='safe_F')
    display = plotting.plot_stat_map(
        z_map_safe, bg_img=mean_img_, threshold=2.5, title=contrast_id,
        vmax=10)


import matplotlib.pyplot as plt
plt.figure()
z1 = np.ravel(z_map.get_data())
z2 = np.ravel(z_map_safe.get_data())
plt.plot(z1, z2, '.')
plt.show()
