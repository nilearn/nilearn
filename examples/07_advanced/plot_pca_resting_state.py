...
#####################################################################
# Preprocess
from nilearn.input_data import NiftiMasker, fmriprep_confounds_strategy
...
components_masked = pca.fit_transform(data_masked.T).T
...
masker = NiftiMasker(smoothing_fwhm=8, memory='nilearn_cache', memory_level=1,
                     mask_strategy='epi', standardize=True)
confounds, sm = fmriprep_confounds_strategy(func_filename, denoise_strategy="simple", global_signal="basic")
data_masked = masker.fit_transform(func_filename, confounds=confounds)
