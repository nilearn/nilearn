...
8-mm-radius spheres centered at coordinates from [1] and estimates partial
...
###############################################################################
# We use the obtained coordinates to label GM voxels less than 8mm from each
# ROI center
...
###############################################################################
# and combine the labels to mark GM voxels part of the whole DMN.
gm_voxels_in_dmn = \
    gm_voxels_in_pcc + gm_voxels_in_mpfc + gm_voxels_in_lag + gm_voxels_in_rag

print('Labeled {0} from the {1} GM voxels as part of DMN.'.format(
    gm_voxels_in_dmn.sum(), gm_voxels_in_dmn.shape[0]))
###############################################################################
# Finally, we need to go back from the mask array to the mask image.
...
print('Saved ROIs time-series in array of shape {0}'.format(
    dmn_time_series.shape))
...
###############################################################################
# We can check that we got a square (n_spheres, n_spheres) matrix.
