...
# Avoid 0 label
ward_labels = ward.labels_ + 1
ward_labels_img = nifti_masker.inverse_transform(ward_labels)

rena_labels = rena.labels_ + 1
permutation = np.random.permutation(rena_labels.shape[0])
rena_labels = permutation[rena_labels]
rena_labels_img = nifti_masker.inverse_transform(rena_labels)


from nilearn.image import mean_img
mean_func_img = mean_img(func_filename)

# first_plot = plot_roi(rena_labels_img, mean_func_img,
#                       title="ReNA parcellation",
#                       display_mode='xz')

# first_plot = plot_roi(ward_labels_img, mean_func_img,
...
first_plot = plot_epi(nifti_masker.inverse_transform(fmri_masked[0]),
...
# dimension
fmri_reduced_ward = ward.transform(fmri_masked)

# Apply this sample mask to X (fMRI data).
...
fmri_compressed_ward = ward.inverse_transform(fmri_reduced_ward)
compressed_img_ward = nifti_masker.inverse_transform(fmri_compressed_ward[0])

fmri_compressed_rena = rena.inverse_transform(fmri_reduced_rena)
compressed_img_rena = nifti_masker.inverse_transform(fmri_compressed_rena[0])


plot_epi(compressed_img_ward, cut_coords=cut_coords,
