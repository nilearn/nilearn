...
- :bdg-info:`Plotting` Add ``node_labels`` parameter to :func:`~plotting.view_connectome` (:gh:`3775` by `Rishika Kaps`_).

- :bdg-info:`Plotting` Add ``surf_mesh`` parameter to :func:`~plotting.view_markers` to support custom whole-brain meshes (:gh:`<your-PR-number>` by `Rishika Kaps`_).
