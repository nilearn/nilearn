...
- Function :func:`~glm.first_level.run_glm` and class :class: `~glm.first_level.first_level.FirstLevelModel` now accept a ``random_state`` parameter, which allows users to seed the ``KMeans`` cluster model used to estimate AR coefficients. (:gh:`3185` by `Sami Jawhar`_).
