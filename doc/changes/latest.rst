...
- Fixes the behavior of :func:`nilearn.decoder.Decoder` where it sets the score to zero if all coefficients are zero, irrespective of the scoring metric used. This change ensures that the average cross-validated scores are comparable to a pure sklearn implementation. (:gh:`5097`).
...
- :bdg-dark:`Code` Do not set score to zero if all coefficients are zero in :func:`nilearn.decoder.Decoder` and ensure average scores are comparable to a pure sklearn implementation (:gh:`5097` by `Himanshu Aggarwal`_).
