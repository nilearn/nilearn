...
- :class:`~input_data.NiftiMasker`, :class:`~input_data.NiftiLabelsMasker`, :class:`~input_data.MultiNiftiMasker`, :class:`~input_data.NiftiMapsMasker`, and :class:`~input_data.NiftiSpheresMasker` can now compute high variance confounds on the images provided to ``transform`` and regress them out automatically. This behaviour is controlled through the ``high_variance_confounds`` boolean parameter of these maskers which default to ``False`` (:gh:`2697` by `Nicolas Gensollen`_).

- :class:`~input_data.NiftiLabelsMasker` now automatically replaces ``NaNs`` in input data with zeros, to match the behavior of other maskers (:gh:`2712` by `Taylor Salo`_).
