...
- :bdg-success:`API` Add ``vmin`` and ``symmetric_cbar`` arguments to :func:`~plotting.plot_img_on_surf` (:gh:`3873` by `Michelle Wang`_).
...
- :bdg-dark:`Code` Make the :func:`~interfaces.fmriprep.load_confounds` confounds file selection more generic (:gh:`3794` by `Taylor Salo`_).

- :bdg-dark:`Code` Relax the :func:`~interfaces.fmriprep.load_confounds` confounds selection on ``cosine`` as not all confound files contained the variables (:gh:`3816` by `Hao-Ting Wang`_).
...
- :bdg-secondary:`Maint` Move the ``~plotting.html_surface._mix_colormaps`` to ``cm.py`` in :mod:`~nilearn.plotting` (:gh:`3919` by `Ahmad Chamma`_)

- :bdg-secondary:`Maint` Remove deprecation decorator from ``~glm.regression`` (:gh:`3921` by `Ahmad Chamma`_)
