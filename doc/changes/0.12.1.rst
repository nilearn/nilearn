...
- :bdg-dark:`Code` :func:`~reporting.make_glm_report` and :func:`~glm.threshold_stats_img` will now warn users when they try to use ``threshold`` with ``height_control`` other than ``None`` (:gh:`5578` by `Rémi Gau`_).
