...
.. admonition:: Major bug
    :class: error

    Edit from September 2026.

    This version contains a major bug that lead maskers casting cleaned signals from float to integers when extracting data from images with an integer dtype. This collapses the cleaned values to a handful of integer levels, destroying almost all of the signal's precision and leading to a decrease in decoding performance (see :gh:`6525`).

    We strongly recommended avoiding using this version (pin ``nilearn!=0.14.0`` or ``nilearn>0.14.0`` in your dependency tracking file) and ideally upgrading to version 0.14.1.
...
- :func:`~image.smooth_img` can now work with surfaces.
...
- :bdg-dark:`Code` Keep values equal to threshold while thresholding image with :func:`~image.threshold_img`.  (:gh:`6130` by `Hande Gözükan`_).

- :bdg-dark:`Code` Do not copy mask image's header in ``masking.unmask`` (:gh:`6157` by `Taylor Salo`_).

- :bdg-dark:`Code` Make sure that area data of Freesurfer can be loaded as :class:`~surface.SurfaceImage` by :func:`~datasets.load_fsaverage_data` (:gh:`6230` by `Rémi Gau`_).
...
- :bdg-dark:`Code` Raise error when maps (or labels) images contain no map (or label) after resampling by :class:`~maskers.NiftiMapsMasker` (or :class:`~maskers.NiftiLabelsMasker`) (:gh:`6240` by `Rémi Gau`_).

- :bdg-dark:`Code` Fix :class:`~maskers.SurfaceMapsMasker` to use ``allow_overlap`` parameter (:gh:`6211` by `Rémi Gau`_ and `Hande Gözükan`_).
...
- :bdg-dark:`Code` Better checks of dtype for :class:`~surface.SurfaceImage` and :class:`~surface.PolyData`. (:gh:`6271` by `Rémi Gau`_).
...
- :bdg-primary:`Doc` Rewrite :class:`~decoding.Decoder` :ref:`example <sphx_glr_auto_examples_02_decoding_plot_haxby_grid_search.py>` with incorrect nested cross-validation implementation (:gh:`6059` by `Michelle Wang`_).
...
- :bdg-secondary:`Maint` The optional dependency ``plotly`` now only install ``plotly``: this reverts a change made in ``0.10.3`` but should allow finer control of what plotting dependencies to install (:gh:`6352` by `Rémi Gau`_).
...
- :bdg-info:`Plotting` Fix bug introduced in ``0.13.0`` while plotting single valued data with :func:`~plotting.plot_roi`.  (:gh:`6122` by `Hande Gözükan`_).

- :bdg-info:`Plotting` Fix colors while plotting ROI contours with ``nilearn.plotting.plot_roi`` (:gh:`6245` by `Hande Gözükan`_).


.. _v0-14-0-enhancements:

Enhancements
------------

- :bdg-success:`API` Allow the ``mask_img`` parameter to :func:`~glm.first_level.first_level_from_bids` to take the value ``"derivatives"`` to generate a mask from the masks in BIDS derivative to use in the model (:gh:`6260` by `Rémi Gau`_).
...
- :bdg-success:`API` Extend the list of estimators supported by :class:`~decoding.SearchLight` (:gh:`6215` by `Rémi Gau`_).

- :bdg-success:`API` Add a public method to access the fitted mask of GLM instances (:gh:`6005` by `Rémi Gau`_).
...
- :bdg-dark:`Code` Add surface support to :func:`~image.smooth_img` (:gh:`3267` by `Jason D. Yeatman`_ and `Noah C. Benson`_ ).

- :bdg-dark:`Code` Add ``screening_n_features`` parameter to :class:`~decoding.Decoder`,  :class:`~decoding.DecoderRegressor`, :class:`~decoding.FREMClassifier`,  and :class:`~decoding.FREMRegressor` (:gh:`5863` by `Sanjana Soni`_).

- :bdg-dark:`Code` Improve type annotations to :mod:`~nilearn.signal`, :mod:`~nilearn.surface`,  :mod:`~nilearn.masking` and :mod:`~nilearn.image` (:gh:`6353`, :gh:`6338` and :gh:`6345` by `Rémi Gau`_).

- :bdg-primary:`Doc` Add jupyterlite option in the documentation (:gh:`6033` by `Hugo Delhaye`_).

- :bdg-primary:`Doc` Add a :ref:`user guide page <gpu_usage>` to demonstrate speedup using GPU (:gh:`5958` by `Himanshu Aggarwal`_ and `Elizabeth DuPre`_).
...
- :bdg-primary:`Doc` Add API documentation for :class:`~connectome.ConnectivityMeasure` to elaborate the requirement for customized ``cov_estimator`` and extra test to ensure the user input is a valid sklearn estimator (:gh:`6074` by `Hao-Ting Wang`_).

- :bdg-primary:`Doc` Add ``Examples`` docstring section for :func:`~plotting.plot_carpet` (:gh:`6065` by `Johanna Bayer`_), :func:`~image.crop_img` (:gh:`6316` by `Thibault de Varax`_), :func:`~connectome.cov_to_corr`, :func:`~connectome.prec_to_partial`, :func:`~glm.expression_to_contrast_vector`, :func:`~glm.fdr_threshold`, :func:`~glm.first_level.glover_dispersion_derivative`, :func:`~glm.first_level.glover_hrf`, :func:`~glm.first_level.glover_time_derivative`, :func:`~glm.first_level.mean_scaling` and :func:`~interfaces.bids.parse_bids_filename` (:gh:`6259` and :gh:`6262` by `Laura Piñero Roig`_), :func:`~masking.compute_background_mask` and :func:`~glm.first_level.make_first_level_design_matrix` (:gh:`6303` and :gh:`6320` by `Marco Flores`_), :func:`~image.largest_connected_component_img` (:gh:`6283` by `Bastien Cagna`_), :func:`~image.threshold_img` (:gh:`6280` by `Fernanda Ponce`_), :func:`~image.clean_img` (:gh:`6312` by `Nirmitee Mulay`_), :func:`~masking.intersect_masks` (:gh:`6318` by `Marco Bedini`_), :func:`~interfaces.bids.get_bids_files` (:gh:`6317` by `Gabriele Amorosino`_), :func:`~image.smooth_img`, :func:`~glm.threshold_stats_img`, :func:`~image.check_niimg`, :func:`~image.check_niimg_3d` and	:func:`~image.check_niimg_4d`, :func:`~regions.signals_to_img_labels` (:gh:`6302`, :gh:`6297`, :gh:`6286` and :gh:`6306`  by `Hande Gözükan`_), :func:`~image.copy_img` (:gh:`6299` by `Ruben Dörfel`_).
