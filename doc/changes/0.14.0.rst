...
- Accessing the maskers from ``nilearn.input_data`` is no longer possible, they now must be accessed via :mod:`~maskers`.
...
- :bdg-dark:`Code` Improve type annotations to :mod:`~signal`, :mod:`~surface`,  :mod:`~masking` and :mod:`~image` (:gh:`6353`, :gh:`6338` and :gh:`6345` by `Rémi Gau`_).
...
- :bdg-danger:`Deprecation` Accessing the maskers from ``nilearn.input_data`` is no longer possible, they now must be accessed via :mod:`~maskers` (:gh:`5995` by `Rémi Gau`_).
