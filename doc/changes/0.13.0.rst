...
- :bdg-dark:`Code` Raise warning when no supra threshold regions were found by :func:`~nilearn.regions.connected_regions` and raise error when :func:`~nilearn.image.concat_imgs` is called with an empty list (:gh:`5909` by `Hande Gözükan`_).
