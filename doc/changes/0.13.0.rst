...
- :bdg-dark:`Code` Raise warning when no supra threshold regions were found by :func:`~nilearn.regions.region_extractor.connected_regions`  and raise error when :func:`~nilearn.image.image.concat_imgs` is called with an empty list (:gh:`5909` by `Hande Gözükan`_).
