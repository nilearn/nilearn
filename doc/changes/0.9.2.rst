...
- :bdg-success:`API` :func:`~glm.save_glm_to_bids` has been added, which writes model outputs to disk according to BIDS convention (:gh:`2715` by `Taylor Salo`_).
...
- :bdg-info:`Plotting` Fix plotting of background image in :func:`~plotting.view_img` when the background is not the MNI template (:gh:`3312` by `Jerome Dockes`_).
...
- :bdg-primary:`Doc` Docstrings of module :mod:`~glm.second_level` were improved (:gh:`3030` by `Nicolas Gensollen`_).
...
- :bdg-secondary:`Maint` Private submodules, functions, and classes from the :mod:`~decomposition` module now start with a "_" character to make it clear that they are not part of the public API (:gh:`3141` by `Nicolas Gensollen`_).
