...
- :bdg-dark:`Code` Fixed bug related to not accepting a list of arrays as an input to unmask, in :mod:`~masking` module.
...
- :bdg-success:`API` New function :func:`~regions.connected_label_regions` to extract the connected components represented as same label to regions apart with each region labeled as unique label.
