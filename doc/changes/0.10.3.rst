...
- :bdg-success:`API` Allow passing arguments to :func:`~glm.first_level.first_level_from_bids` to build first level models that include specific set of confounds by relying on the strategies from :func:`~interfaces.fmriprep.load_confounds` (:gh:`4103` by `Rémi Gau`_).

- :bdg-success:`API` Support passing t and F contrasts to :func:`~glm.compute_contrast` that that have fewer columns than the number of estimated parameters. Remaining columns are padded with zero (:gh:`4067` by `Rémi Gau`_).
...
- :bdg-info:`Plotting` Allow setting ``vmin`` in :func:`~plotting.plot_glass_brain` and :func:`~plotting.plot_stat_map` (:gh:`3993` by `Michelle Wang`_).
...
- :bdg-dark:`Code` Fix :class:`~glm.regression.SimpleRegressionResults` to accommodate for the lack of a ``model`` attribute (:gh:`4071` `Rémi Gau`_)
...
- :bdg-dark:`Code` Fix PSC normalization method applied by ``standardize_signal`` in :mod:`~signal` (:gh:`4116` by `Hao-Ting Wang`_ and `Yasmin Mzayek`_).
...
- :bdg-danger:`Deprecation` Make sure that all the fetchers for ``atlas`` and ``func`` from :mod:`~datasets` return a :class:`sklearn.utils.Bunch`. Add new behavior to :func:`~datasets.fetch_language_localizer_demo_dataset` to return output as ``Bunch``. This behavior will become the default in version 0.13.0 (:gh:`4233` by `Rémi Gau`_).
...
- :bdg-success:`API` Allow passing Pandas Series of image filenames to :class:`~glm.second_level.SecondLevelModel` (:gh:`4070` by `Rémi Gau`_).
...
- :bdg-dark:`Code` Add ``two_sided`` option for :class:`~image.binarize_img` (:gh:`4121` by `Steven Meisler`_).
...
- :bdg-primary:`Doc` Add backslash to homogenize :class:`~regions.Parcellations` documentation (:gh:`4042` by `Nikhil Krish`_).
...
- :bdg-primary:`Doc` Describe the return value in :func:`~datasets.fetch_abide_pcp` documentation (:gh:`4159` by `Suramya Pokharel`_).
