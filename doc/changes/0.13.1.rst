...
- This is mostly a release to fix some bugs introduced in ``0.13.0`` and to ensure better compatibility with scikit-learn ``1.8.0``.
...
- :bdg-dark:`Code` Only throw warning about non-interactive plotting backend when using :func:`~plotting.show` (:gh:`5929` by `Rémi Gau`_).

- :bdg-dark:`Code` Re-allow use non-multi maskers for :class:`~regions.Parcellations` (:gh:`5930` by `Rémi Gau`_).
