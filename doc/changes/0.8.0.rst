...
- Class :class:`~input_data.NiftiLabelsMasker` can now generate HTML reports in the same way as the :class:`~input_data.NiftiMasker` (:gh:`2707` by `Nicolas Gensollen`_).
...
- All inherent classes of ``nilearn.input_data.BaseMasker`` can use the parameter ``sample_mask`` for sub-sample masking (:gh:`2858` by `Hao-Ting Wang`_).
...
- Class :class:`~input_data.NiftiLabelsMasker` can now generate HTML reports in the same way as :class:`~input_data.NiftiMasker`. The report shows the regions defined by the provided label image and provide summary statistics on each region (name, volume...). If a functional image was provided to fit, the middle image is plotted with the regions overlaid as contours. Finally, if a mask is provided, its contours are shown in green (:gh:`2707` by `Nicolas Gensollen`_).
...
- Moves parameter ``sample_mask`` from :class:`~input_data.NiftiMasker` to method ``transform`` in base class `nilearn.input_data.BaseMasker` (:gh:`2858` by `Hao-Ting Wang`_).
...
- Class :class:`~input_data.NiftiMasker` replaces attribute ``sessions`` with ``runs`` and deprecates attribute ``sessions`` in ``0.9.0``. Match the relevant change in function :func:`~signal.clean` (:gh:`2858` by `Hao-Ting Wang`_).
