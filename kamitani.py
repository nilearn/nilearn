...
if multi_scale and learn_fusion_params:
    ...
    t_pred = t_preds[0]
    t_pred_tall = np.array([np.dot(height_transform.T * 2,
        np.reshape(m, (5, 10))).flatten() for m in t_preds[1]])
    t_pred_large = np.array([np.dot(np.reshape(m, (10, 5)),
        width_transform.T * 2).flatten() for m in t_preds[2]])
    t_pred_big = [np.dot(height_transform.T * 2, np.reshape(m, (5, 5)))
        for m in t_preds[3]]
    t_pred_big = np.array([np.dot(np.reshape(m, (10, 5)),
        width_transform.T * 2).flatten() for m in t_pred_big])
