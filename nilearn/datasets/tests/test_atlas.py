...
def test_downloader(temp_dir_path):
    ...
    datasetdir = os.path.join(temp_dir_path, 'craddock_2012')
    ...
    pytest.raises(IOError, utils._fetch_files,
                  os.path.join(temp_dir_path, 'craddock_2012'), files,
                  verbose=0)
    ...
    atlas.fetch_atlas_craddock_2012(data_dir=temp_dir_path, url=local_url)
    dummy = open(os.path.join(datasetdir, 'random_all.nii.gz'), 'r')
    stuff = dummy.read()
    dummy.close()
    assert stuff == ''
...
def test_fail_fetch_atlas_harvard_oxford(temp_dir_path):
    ...
    ho_dir = os.path.join(temp_dir_path, 'fsl', 'data', 'atlases')
    os.makedirs(ho_dir)
...
def test_fetch_atlas_craddock_2012(temp_dir_path):
    bunch = atlas.fetch_atlas_craddock_2012(data_dir=temp_dir_path,
                                            verbose=0)
    ...
    for key, fn in zip(keys, filenames):
        assert bunch[key] == os.path.join(temp_dir_path, 'craddock_2012', fn)
    assert bunch.description != ''


@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_smith_2009(temp_dir_path):
    bunch = atlas.fetch_atlas_smith_2009(data_dir=temp_dir_path, verbose=0)
    ...
    for key, fn in zip(keys, filenames):
        assert bunch[key] == os.path.join(temp_dir_path, 'smith_2009', fn)
    assert bunch.description != ''
...
@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_destrieux_2009(temp_dir_path):
    datadir = os.path.join(temp_dir_path, 'destrieux_2009')
    os.mkdir(datadir)
    dummy = open(os.path.join(
        datadir, 'destrieux2009_rois_labels_lateralized.csv'), 'w')
    dummy.write("name,index")
    dummy.close()
    bunch = atlas.fetch_atlas_destrieux_2009(data_dir=temp_dir_path,
                                             verbose=0)

    assert len(tst.mock_url_request.urls) == 1
    assert bunch['maps'] == os.path.join(
        temp_dir_path, 'destrieux_2009', 'destrieux2009_rois_lateralized.nii.gz')
    ...
    bunch = atlas.fetch_atlas_destrieux_2009(
        lateralized=False, data_dir=temp_dir_path, verbose=0)

    assert len(tst.mock_url_request.urls) == 1
    assert bunch['maps'] == os.path.join(
        temp_dir_path, 'destrieux_2009', 'destrieux2009_rois.nii.gz')


@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_msdl(temp_dir_path):
    datadir = os.path.join(temp_dir_path, 'msdl_atlas')
    os.mkdir(datadir)
    ...
    dataset = atlas.fetch_atlas_msdl(data_dir=temp_dir_path, verbose=0)
...
@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_yeo_2011(temp_dir_path):
    dataset = atlas.fetch_atlas_yeo_2011(data_dir=temp_dir_path, verbose=0)
...
@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_aal(temp_dir_path):
    ho_dir = os.path.join(temp_dir_path, 'aal_SPM12', 'aal', 'atlas')
    os.makedirs(ho_dir)
    with open(os.path.join(ho_dir, 'AAL.xml'), 'w') as xml_file:
        xml_file.write("<?xml version='1.0' encoding='us-ascii'?> "
                       "<metadata>"
                       "</metadata>")
    dataset = atlas.fetch_atlas_aal(data_dir=temp_dir_path, verbose=0)
    assert isinstance(dataset.maps, _basestring)
    assert isinstance(dataset.labels, list)
    ...
    assert_raises_regex(ValueError, 'The version of AAL requested "FLS33"',
                        ...
                        data_dir=temp_dir_path, verbose=0)

    assert dataset.description != ''


@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_basc_multiscale_2015(temp_dir_path):
    # default version='sym'
    data_sym = atlas.fetch_atlas_basc_multiscale_2015(data_dir=temp_dir_path,
                                                      verbose=0)
    # version='asym'
    ...
    for key, basename_sym in zip(keys, basenames_sym):
        assert data_sym[key] == os.path.join(temp_dir_path, dataset_name,
    ...
    for key, basename_asym in zip(keys, basenames_asym):
        assert data_asym[key] == os.path.join(temp_dir_path, dataset_name,
...
@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_allen_2011(temp_dir_path):
    bunch = atlas.fetch_atlas_allen_2011(data_dir=temp_dir_path, verbose=0)
    keys = ("maps",
            "rsn28",
            "comps")
    ...
    assert len(tst.mock_url_request.urls) == 1
    for key, fn in zip(keys, filenames):
        assert bunch[key] == os.path.join(temp_dir_path, 'allen_rsn_2011',
                                              'allen_rsn_2011', fn)
...
@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_surf_destrieux(temp_dir_path, verbose=0):
    data_dir = os.path.join(temp_dir_path, 'destrieux_surface')
    os.mkdir(data_dir)
    # Create mock annots
    ...
    bunch = atlas.fetch_atlas_surf_destrieux(data_dir=temp_dir_path, verbose=0)
...
@with_setup(setup_mock, teardown_mock)
def test_fetch_atlas_talairach(temp_dir_path):
    atlas._fetch_files = _mock_talairach_fetch_files
    level_values = np.ones((81, 3)) * [0, 1, 2]
    talairach = atlas.fetch_atlas_talairach('hemisphere', data_dir=temp_dir_path)
    assert_array_equal(get_data(talairach.maps).ravel(),
                       level_values.T.ravel())
    assert_array_equal(talairach.labels, ['Background', 'b', 'a'])
    talairach = atlas.fetch_atlas_talairach('ba', data_dir=temp_dir_path)
    assert_array_equal(get_data(talairach.maps).ravel(),
                       level_values.ravel())
    pytest.raises(ValueError, atlas.fetch_atlas_talairach, 'bad_level')


def test_fetch_atlas_pauli_2017(temp_dir_path):
    data_dir = os.path.join(temp_dir_path, 'pauli_2017')
...
def test_fetch_atlas_schaefer_2018(temp_dir_path):
