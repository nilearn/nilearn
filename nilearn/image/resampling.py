...
def compare_affines(source_affine, target_affine, full_matrices=False):
    """Compares the spaces encoded by two affine matrices. Returns an
    expression of target affine as source affine times axis permutation
    times voxel size diagonal times remainder plus integer offset plus
    non integer offset

    Parameters
    ==========

        source_affine: ndarray, shape=(4, 4)
            affine encoding ambient space

        target_affine: ndarray, shape=(4, 4)
            affine encoding a second space, to be expressed by the first

        full_matrices: boolean, default False
            return a list of matrices that create the transformation by
            multiplying them together, instead of a list of object that
            encode the transformation steps minimally.
    """

    transformation_matrix = np.linalg.inv(target_affine).dot(source_affine)
    QT, RT = np.linalg.qr(transformation_matrix[:3, :3].T)
    R, Q = RT.T, QT.T

    # 0) Make sure lower triangular matrix has positive diagonal
    R_diag_sign = np.sign(np.diag(R))
    Q *= R_diag_sign[:, np.newaxis]
    R *= R_diag_sign
    ...
    if len(np.unique(strongest_axes)) != 3:
        ...
        Q_copy = Q.copy()
        first_axis = np.argmax(np.abs(Q[0]))
        Q_copy[:, first_axis] = 0
        second_axis = np.argmax(np.abs(Q_copy[1]))
        Q_copy[:, second_axis] = 0
        third_axis = np.argmax(np.abs(Q_copy[2]))
        strongest_axes = np.array([first_axis, second_axis, third_axis])

    permutation = strongest_axes
    P = np.zeros([3, 3])
    P[[0, 1, 2], permutation] = 1

    # 1.5) Factor out the permutation from the orthogonal matrix Q
    Q = Q.dot(np.linalg.inv(P))

    # 2) Find sign inversions
    sign_inversions = np.sign(np.diag(R))
    SI = np.diag(sign_inversions)
    R = R.dot(SI)

    # 3) Find relative voxel sizes
    voxel_sizes = np.diag(R)
    V = np.diag(voxel_sizes)

    # 3.5) Factor out voxel sizes from R
    R = R.dot(np.linalg.inv(V))
    # Residual shear is now left in R
    ...
    T_int_full = np.eye(4)
    T_int_full[:3, 3] = V.dot(translation_int)

    T_frac_full = np.eye(4)
    T_frac_full[:3, 3] = V.dot(translation_frac)
