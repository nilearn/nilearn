...
def test_view_img():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        _check_html(html_view)
        html_view = view_img(img, threshold=2.0, vmax=4.0)
        _check_html(html_view)
        html_view = view_img(img, symmetric_cmap=False)
        img_4d = image.new_img_like(img, get_data(img)[:, :, :, np.newaxis])
        assert len(img_4d.shape) == 4
        html_view = view_img(img_4d, threshold=2.0, vmax=4.0)
        _check_html(html_view)
        html_view = view_img(img_4d, threshold=1e6)
        _check_html(html_view)
        html_view = view_img(img_4d, width_view=1000)
        _check_html(html_view)
