...
def test_get_vertexcolor():
    ...
    assert (
        np.min(bg_map_normalized) == 0 and np.max(bg_map_normalized) == 1
    )
...
def test_view_img_on_surf():
    ...
    html = html_surface.view_img_on_surf(
        img, threshold=0, surf_mesh=surfaces
    )
    check_html(html)
    html = html_surface.view_img_on_surf(
        img, threshold=0.4, title="SOME_TITLE"
    )
    assert "SOME_TITLE" in html.html
    check_html(html)
    html = html_surface.view_img_on_surf(
        img, threshold=0.4, cmap="hot", black_bg=True
    )
    check_html(html)
