...
@pytest.mark.skipif(PLOTLY_INSTALLED, reason="Plotly is installed.")
...
@pytest.mark.skipif(
    not PLOTLY_INSTALLED or KALEIDO_INSTALLED,
...
@pytest.mark.skipif(
    not PLOTLY_INSTALLED or not KALEIDO_INSTALLED,
...
@pytest.mark.skipif(
    not PLOTLY_INSTALLED or not IPYTHON_INSTALLED,
    reason=(
...
def test_plot_surf_stat_map_error():
    ...
    # Try to input vmin
    with pytest.raises(
        ValueError, match='this function does not accept a "vmin" argument'
    ):
...
def test_plot_surf_roi_matplotlib_specific():
    ...
    # plot to axes
    plot_surf_roi(
        mesh, roi_map=roi_map, ax=None, figure=plt.gcf(), engine="matplotlib"
    )
...
def test_plot_img_on_surf_hemispheres_and_orientations():
    ...
    plot_img_on_surf(nii, hemispheres=["right"], views=["medial", "lateral"])
    plot_img_on_surf(
        nii, hemispheres=["left", "right"], views=["dorsal", "medial"]
    )
...
def test_plot_img_on_surf_colorbar():
    ...
    plot_img_on_surf(
        nii, hemispheres=["right"], views=["lateral"], colorbar=False
    )
    plot_img_on_surf(
        nii,
        hemispheres=["right"],
...
def test_plot_img_on_surf_inflate():
    nii = _generate_img()
    plot_img_on_surf(
        nii, hemispheres=["right"], views=["lateral"], inflate=True
    )
...
def test_plot_img_on_surf_title():
    nii = _generate_img()
    title = "Title"
    fig, axes = plot_img_on_surf(nii, hemispheres=["right"], views=["lateral"])
