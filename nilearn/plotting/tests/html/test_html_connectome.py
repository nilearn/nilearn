...
def test_prepare_colors_for_markers(node_color, expected_marker_colors):
    number_of_nodes = 3
    marker_colors = _prepare_colors_for_markers(
        node_color,
        number_of_nodes,
    )
...
def test_get_connectome():
    ...
    connectome = _get_connectome(adj, coord)
...
def test_view_markers(tmp_path):
    ...
    html = view_markers(coords, colors)
    check_html(tmp_path, html, False, "connectome-plot")
    html = view_markers(coords)
    check_html(tmp_path, html, False, "connectome-plot")
