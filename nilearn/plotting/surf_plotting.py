"""
Functions for surface visualization using matplotlib.
"""
...
def plot_surf(surf_mesh, surf_map=None, bg_map=None,
    ...
    surf_mesh : :obj:`str` or :obj:`list` of two :class:`numpy.ndarray`
        Surface mesh geometry, can be a file (valid formats are .gii or Freesurfer
        specific files such as .orig, .pial, .sphere, .white, .inflated) or a list
        of two Numpy arrays, the first containing the x-y-z coordinates of the mesh
        vertices, the second containing the indices (into coords) of the mesh faces.

    surf_map : :obj:`str` or :class:`numpy.ndarray`, optional.
        ...
        .thickness, .curv, .sulc, .annot, .label) or a Numpy array with a value
        for each vertex of the surf_mesh.

    bg_map : Surface data object (to be defined), optional,
        Background image to be plotted on the mesh underneath the surf_data in
        greyscale, most likely a sulcal depth map for realistic shading.
    ...
    colorbar : :obj:`bool`, optional, default is False
    ...
    alpha : :obj:`float`, alpha level of the mesh (not surf_data), default 'auto'
        If 'auto' is chosen, alpha will default to .5 when no bg_map
        is passed and to 1 if a bg_map is passed.

    bg_on_data : :obj:`bool`, default is False
    ...
    darkness : :obj:`float`, between 0 and 1, default is 1
        Specifying the darkness of the background image.
        1 indicates that the original values of the background are used.
        0.5 indicates the background values are reduced by half before being
        applied.
    ...
    if hemi == 'right':
        ...
        if view == 'medial':
            ...
            raise ValueError('Value for view must be one of lateral, medial, '
                             'dorsal, ventral, anterior, or posterior')
    else:
        raise ValueError('Value for hemi must be one of right or left')
