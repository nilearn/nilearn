...
def test_load_surf_mesh_file_gii(tmp_path):
    ...
    os.close(fd_no)
    nb.save(
        gifti.GiftiImage(darrays=[face_array, face_array]),
    ...
    os.close(fd_face)
    nb.save(
        gifti.GiftiImage(darrays=[coord_array, coord_array]),
...
def test_load_surf_mesh_file_freesurfer(tmp_path):
    ...
    for suff in [".pial", ".inflated", ".white", ".orig", "sphere"]:
        fd, filename_fs_mesh = tempfile.mkstemp(suffix=suff, dir=str(tmp_path))
        os.close(fd)
...
def test_projection_matrix():
    ...
    proj = surface._projection_matrix(
        mesh, np.eye(4), img.shape, radius=2.0, n_points=10
    )
    ...
    mesh = flat_mesh(5, 7)
    proj = surface._projection_matrix(
        mesh, np.eye(4), (5, 7, 1), radius=0.1, n_points=10
    )
    assert_array_almost_equal(proj.toarray(), np.eye(proj.shape[0]))
    mask = np.ones(img.shape, dtype=int)
    mask[0] = 0
    proj = surface._projection_matrix(
        mesh, np.eye(4), img.shape, radius=2.0, n_points=10, mask=mask
    )
    proj = proj.toarray()
...
def test_sampling_affine():
    ...
    assert_array_equal(texture[0], [1.0, 2.0, 1.0])
    texture = surface._interpolation_sampling(
        [img], mesh, affine=affine, radius=0, kind="ball"
    )
    assert_array_almost_equal(texture[0], [1.1, 2.0, 1.0])
...
def test_sampling(kind, use_inner_mesh, projection):
    ...
    projection = projector(
        [img], mesh, np.eye(4), kind=kind, radius=0.0, inner_mesh=inner_mesh
    )
