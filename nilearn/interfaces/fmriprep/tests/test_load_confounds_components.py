...
from nilearn.interfaces.fmriprep import (
    load_confounds,
    load_confounds_components as components,
)
...
@pytest.mark.parametrize(
    "component", ["motion", "wm_csf", "global_signal", "compcor", "ica_aroma"]
)
def test_missing_keywords(tmp_path, component):
    ...
    if component in ["motion", "wm_csf", "global_signal"]:
        remove_columns = getattr(components, f"_load_{component}")(
            legal_confounds, "full"
        )
        ...
        remove_columns = _find_compcor(
            meta_json, compcor="anat_combined", n_compcor=6
        )
    elif component == "ica_aroma":
        legal_confounds = pd.read_csv(
            bad_conf, delimiter="\t", encoding="utf-8"
        )
        remove_columns = getattr(components, f"_load_{component}")(
            legal_confounds, "basic"
        )
    else:
        remove_columns = getattr(components, f"_load_{component}")(
            legal_confounds
    ...
    with pytest.raises(TypeError) as exc_info:
        getattr(components, f"_load_{component}")(confounds_raw)
