...
class _ExtractionFunctor(object):
    ...
    def __init__(
        self, _resampled_labels_img_, background_label, strategy, mask_img
    ):
...
class NiftiLabelsMasker(BaseMasker, CacheMixin):
    ...
    def fit(self, X=None, y=None):
        ...
        logger.log(
            "loading data from %s"
            % _utils._repr_niimgs(self.labels_img, shorten=(not self.verbose)),
        ...
        if self.mask_img_ is not None:
            if self.resampling_target == "data":
                ...
                if not np.allclose(
                    self.mask_img_.affine, self.labels_img_.affine
                ):
                    raise ValueError(
                        _compose_err_msg(
    ...
    def transform_single_imgs(self, imgs, confounds=None, sample_mask=None):
        ...
        params = get_params(
            NiftiLabelsMasker, self, ignore=['resampling_target']
        )
