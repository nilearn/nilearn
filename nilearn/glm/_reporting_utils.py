...
class GLMReportMixin(ReportMixin):
    ...
    def _set_report_basics(self, engine, title):
        ...
        report_content["smoothing_fwhm"] = (
            None if smoothing_fwhm == 0 else smoothing_fwhm
        )
        report_content["is_notebook"] = is_notebook()

    def _run_report_checks(self, **kwargs):
        super()._run_report_checks(**kwargs)
        height_control = kwargs.get("height_control", "fpr")
        cluster_threshold = kwargs.get("cluster_threshold", 0)
        min_distance = kwargs.get("min_distance", 8.0)
        plot_type = kwargs.get("plot_type", "slice")
    ...
    def generate_report(
        ...
        kwargs : :obj:`dict` [ :obj:`str` , Any]
            ...
            Expected keys are for both ``FirstLevelModel`` and
            ``SecondLevelModel`` :
            ...
            bg_img : Niimg-like object, default='MNI152TEMPLATE'
                ...
                The background image for mask and stat maps to be plotted on
                upon. To turn off background image, just pass "bg_img=None".
            ...
            threshold : :obj:`float` or :obj:`int` or None, default=None
                ...
                If ``threshold`` is set to None when ``height_control`` is
                None, ``threshold`` will be set to 3.09.
                ...
                .. note::
                    ...
                    - When ``two_sided`` is True:
                      ...
                      All intensities in the interval
                      ``(-threshold, threshold)`` will be set to zero.
                    ...
                    - When ``two_sided`` is False:
                      ...
                      - If the threshold is negative:
                        ...
                        of the input data.
                        All intensities greater than the specified threshold
                        will be set to zero.
                      ...
                      - If the threshold is positive:
                        ...
                        of the input data.
                        All intensities less than the specified threshold will
                        be set to zero.
                        All other intensities keep their original values.

            alpha : :obj:`float`, default=0.001
                Number controlling the thresholding (either a p-value or
                q-value). Its actual meaning depends on the height_control
                parameter. This function translates alpha to a z-scale
                threshold.
            ...
            two_sided : :obj:`bool`, default=False
                Whether to employ two-sided thresholding or to evaluate
            ...
            plot_type : :obj:`str`, {'slice', 'glass'}, default='slice'
                Specifies the type of plot to be drawn for the statistical
                maps.
    ...
    def _load_bg_img(self, bg_img, is_volume_glm):
...
def mask_to_plot(model, bg_img):
    ...
    """
    if not is_matplotlib_installed() or not model.__sklearn_is_fitted__():
        return None
...
def make_stat_maps_contrast_clusters(
    ...
    check_params(locals())
    stat_img = model._get_report_statistical_maps(
        contrasts, output, first_level_contrast
    )
...
def get_runwise_dict(contrasts, output, design_matrices):
