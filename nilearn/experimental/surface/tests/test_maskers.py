...
def test_mask_img_fit_shape_mismatch(
    flip_img, mini_surface_mask, make_mini_surface_img, assert_img_equal
):
    img = make_mini_surface_img()
    masker = SurfaceMasker(mini_surface_mask)
    ...
    # timeseries) is ok
    masker.fit(make_mini_surface_img((2,)))
    assert_img_equal(mini_surface_mask, masker.mask_img_)


def test_mask_img_fit_keys_mismatch(mini_surface_mask, drop_img_part):
    masker = SurfaceMasker(mini_surface_mask)
    with pytest.raises(ValueError, match="key"):
        masker.fit(drop_img_part(mini_surface_mask))


def test_none_mask_img(mini_surface_mask):
    masker = SurfaceMasker(None)
    with pytest.raises(ValueError, match="provide either"):
        masker.fit(None)
    # no mask_img but fit argument is ok
    masker.fit(mini_surface_mask)
    # no fit argument but a mask_img is ok
    SurfaceMasker(mini_surface_mask).fit(None)


def test_unfitted_masker(mini_surface_mask):
    masker = SurfaceMasker(mini_surface_mask)
    with pytest.raises(ValueError, match="fitted"):
        masker.transform(mini_surface_mask)


def test_mask_img_transform_shape_mismatch(
    flip_img, mini_surface_img, mini_surface_mask
):
    masker = SurfaceMasker(mini_surface_mask).fit()
    with pytest.raises(ValueError, match="number of vertices"):
        masker.transform(flip_img(mini_surface_img))
    # non-flipped is ok
    masker.transform(mini_surface_img)


def test_mask_img_transform_keys_mismatch(
    mini_surface_mask, mini_surface_img, drop_img_part
):
    masker = SurfaceMasker(mini_surface_mask).fit()
    with pytest.raises(ValueError, match="key"):
        masker.transform(drop_img_part(mini_surface_img))
    # full img is ok
    masker.transform(mini_surface_img)
...
def test_transform_inverse_transform(
    shape, make_mini_surface_img, assert_img_equal
):
    img = make_mini_surface_img(shape)
...
def test_transform_inverse_transform_with_mask(
    shape, make_mini_surface_img, assert_img_equal, mini_surface_mask
):
    img = make_mini_surface_img(shape)
    masker = SurfaceMasker(mini_surface_mask).fit(img)
    masked_img = masker.transform(img)
