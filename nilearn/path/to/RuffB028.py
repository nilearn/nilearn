...
warnings.warn("Affine is different across subjects. Realignment on first subject", stacklevel=2)
warnings.warn("Random walker only segments unlabeled areas, where labels == 0. No zero valued areas in labels were found.", stacklevel=2)
warnings.warn("Random walker only segments unlabeled areas, where labels == 0. No zero valued areas in labels were found.", stacklevel=2)
warnings.warn("Random walker received no seed label. Returning provided labels.", stacklevel=2)
warnings.warn("Random walker only segments unlabeled areas, where labels == 0. Data provided only contains isolated seeds.", stacklevel=2)
warnings.warn(f"Couldn't find {mni152_brain_mask} (for testing)", stacklevel=2)
warnings.warn(f"Maximum number of iterations {max_iter} reached without getting to the requested tolerance level {tol}.", stacklevel=2)
warnings.warn("input signals do not all have unit variance. This can lead to numerical instability.", stacklevel=2)
warnings.warn(msg, RuntimeWarning, stacklevel=2)
warnings.warn("Newton-Raphson step did not converge. This may indicate a badly conditioned model.", stacklevel=2)
warnings.warn("Maximum number of iterations reached without getting to the requested tolerance level.", stacklevel=2)
...
warnings.warn("Deprecation warning for fetcher. In release 0.13, this fetcher will return a dictionary.", stacklevel=2)
warnings.warn("In release 0.13, this fetcher will return a dictionary. Please update your code.", category=DeprecationWarning, stacklevel=2)
warnings.warn(message, DeprecationWarning, stacklevel=2)
warnings.warn("In release 0.13, this fetcher will return a dictionary. Please update your code.", category=DeprecationWarning, stacklevel=2)
warnings.warn(f"Warning: there are only {max_subjects} subjects.", stacklevel=2)
warnings.warn("Wrong value for 'n_subjects'. The maximum value will be used instead ('n_subjects=94').", stacklevel=2)
warnings.warn(
