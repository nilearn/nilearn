...
def test_multi_pca():
    ...
    multi_pca = _MultiPCA(
        mask=MultiNiftiMasker(mask_args=dict(opening=0)), do_cca=False, n_components=3
    )
...
def test_multi_pca_score():
    ...
    multi_pca = _MultiPCA(mask=mask_img, random_state=0, memory_level=0, n_components=5)
    multi_pca.fit(imgs[0])
    s = multi_pca.score(imgs[0])
...
def test_components_img():
    ...
    mask_img = nibabel.Nifti1Image(np.ones(shape[:3], dtype=np.int8), affine)
    n_components = 3
    multi_pca = _MultiPCA(mask=mask_img, n_components=n_components, random_state=0)
...
def test_with_globbing_patterns_with_multiple_images():
    ...
    with write_tmp_imgs(img_4d, img_4d, create_files=True, use_wildcards=True) as imgs:
