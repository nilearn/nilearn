...
url_doc = "https://nilearn.github.io/stable/introduction.html#installation"
_NILEARN_INSTALL_MSG = f"See {url_doc} for installation information."
import operator
...
def _import_module_with_version_check(
    ...
    try:
        ...
        if not install_info:
            install_info = "Please install it properly to use nilearn."
        user_friendly_info = (
            f"Module '{module_name}' could not be found. {install_info}"
        )
...
def _compare_version(version_a, operator, version_b):
    ...
    if operator not in VERSION_OPERATORS:
        raise ValueError(
            f"'_compare_version' received an unexpected operator {operator}."
        )
