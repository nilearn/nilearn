...
def test_errors(affine_eye, shape_3d_default, n_regions, length):
    ...
    masker11 = NiftiLabelsMasker(
        labels11_img, resampling_target=None, standardize=None
    )
...
def test_no_label_errors(img_3d_zeros_eye):
...
def test_mask_img_masks_all_labels_error(
...
def test_reduction_strategies(
    affine_eye, strategy, function, test_values, estimator
):
    ...
    input_img = Nifti1Image(img_data, affine_eye)
    if isinstance(masker, MultiNiftiLabelsMasker):
        input_img = [input_img] * 2

    results = masker.fit_transform(input_img)

    if not isinstance(masker, MultiNiftiLabelsMasker):
        results = [results]
    ...
    default_masker = estimator(labels, standardize=None)
    assert default_masker.strategy == "mean"


def test_errors_strategy(img_labels):
    """Test strategy errors."""
    masker = NiftiLabelsMasker(img_labels, strategy="TESTRAISE")
    with pytest.raises(ValueError, match="'strategy' must be one of"):
        masker.fit()


@pytest.mark.parametrize("resampling_target", ["mask", "invalid"])
def test_errors_resampling(img_labels, resampling_target):
...
def test_atlas_data_different_fov(img_labels, affine_eye, length):
    """Test resampling to data in NiftiLabelsMasker.

    The atlas and mask should be resampled to the data by default.
    """
    shape2 = (8, 9, 10)
    _, mask22_img = generate_fake_fmri(shape2, affine=affine_eye * 3)
    ...
    with pytest.raises(AssertionError):
        assert_array_equal(mask22_img.affine, fmri22_img.affine)
    with pytest.raises(AssertionError):
    ...
    assert_array_equal(masker.mask_img_.affine, fmri22_img.affine)
...
def test_resampling_clipped_labels(
    affine_eye, n_regions, length, img_labels, img_fmri, estimator
):
    ...
    _, mask_img = generate_fake_fmri(
        mask_shape, affine=affine_eye, length=length
    )
...
def test_regions_id_names_no_labels_no_lut(affine_eye, shape_3d_default):
    ...
    masker = NiftiLabelsMasker(atlas, standardize=None)
...
def test_regions_id_names_with_labels(
    ...
    masker = NiftiLabelsMasker(atlas, labels=labels, standardize=None)
...
def test_regions_id_names_with_too_few_labels(affine_eye):
    ...
    with pytest.warns(UserWarning, match="Too many indices for the names."):
        ...
        masker = NiftiLabelsMasker(
            atlas, labels=["Background", "A", "B"], standardize=None
        ).fit()
...
def test_regions_id_names_lut(affine_eye, shape_3d_default):
    ...
    masker = NiftiLabelsMasker(atlas, lut=lut, standardize=None)
...
def test_regions_id_names_lut_too_few(affine_eye, shape_3d_default):
    ...
    masker = NiftiLabelsMasker(atlas, lut=lut, standardize=None).fit()
...
def test_pass_lut(
    ...
    masker = NiftiLabelsMasker(img_labels, lut=lut, standardize=None)
    ...
    masker = NiftiLabelsMasker(img_labels, lut=lut_file, standardize=None)
...
def test_no_background(n_regions, img_labels, shape_3d_default, affine_eye):
    ...
    masker = NiftiLabelsMasker(
        img_labels, lut=lut, background_label=999, standardize=None
    )
...
def test_lut_shift(lut):
    ...
    masker = NiftiLabelsMasker(
        labels_img=labels_img, lut=lut, standardize=None
    ).fit()
