...
def test_mask_img_fit_shape_mismatch(flip_img, mini_mask, make_mini_img):
    img = make_mini_img()
    ...
    assert_surf_img_equal(mini_mask, masker.mask_img_)


def test_mask_img_fit_keys_mismatch(mini_mask):
    masker = SurfaceMasker(mini_mask)
    with pytest.raises(ValueError, match="key"):
        masker.fit(drop_hemisphere_surf_img(mini_mask))
...
def test_mask_img_transform_keys_mismatch(mini_mask, mini_img):
    masker = SurfaceMasker(mini_mask).fit()
    with pytest.raises(ValueError, match="key"):
        masker.transform(drop_hemisphere_surf_img(mini_img))
    # full img is ok
    masker.transform(mini_img)


@pytest.mark.parametrize("shape", [(), (1,), (3,), (3, 2)])
def test_transform_inverse_transform(shape, make_mini_img):
