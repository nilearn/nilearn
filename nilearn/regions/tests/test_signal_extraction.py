...
def test_signals_extraction_with_labels():
    ...
    one_d_input = signal_extraction.signals_to_img_labels(signals[0, :], labels_img)
    assert one_d_input.shape == shape + (1,)
    two_d_input = signal_extraction.signals_to_img_labels(signals[:1, :], labels_img)
    assert np.all(get_data(one_d_input) == get_data(two_d_input))
