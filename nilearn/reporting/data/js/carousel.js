...
class Carousel {
  ...
  init () {
    ...
    const prevButton = document.querySelector(`#prev-btn-${this.uid}`)
    const nextButton = document.querySelector(`#next-btn-${this.uid}`)
  ...
  showObj (index) {
    this.displayed_objects.forEach((_, i) => {
      const mapElement = document.getElementById(`carousel-obj-${this.uid}-${i}`)
      if (mapElement) {
        mapElement.style.display = i === index ? 'block' : 'none'
      }
    })

    const compElement = document.getElementById(`comp-${this.uid}`)
    if (compElement) {
    ...
    const matrixNavBar = document.querySelector(`#navbar-matrix-link-${this.uid}`)
    if (matrixNavBar) {
      matrixNavBar.href = `#design-matrix-${this.uid}-${index}`
    }
    const contrastNavBar = document.querySelector(`#navbar-contrasts-link-${this.uid}`)
    if (contrastNavBar) {
