...
def check_masker_transformer(estimator):
    ...
    - also checks that "X" is the parameter for input for fit / transform
    """
    if accept_niimg_input(estimator):
        input_img = _img_4d_rand_eye_medium()
    else:
    ...
    signal_1 = estimator.fit_transform(X=input_img)

    estimator = clone(estimator)
    signal_2 = estimator.fit(X=input_img).transform(X=input_img)
