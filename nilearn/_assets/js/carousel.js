...
class Carousel {
  ...
  init () {
    ...
    const prevButton = document.querySelector(`#prev-btn-UUID-${this.uid}`)
    const nextButton = document.querySelector(`#next-btn-UUID-${this.uid}`)
  ...
  showObj (index) {
    ...
    const matrixNavBar = document.querySelector(`#navbar-matrix-link-UUID-${this.uid}`)
    if (matrixNavBar) {
      matrixNavBar.href = `#design-matrix-UUID-${this.uid}-${index}`
    }
    const contrastNavBar = document.querySelector(`#navbar-contrasts-link-UUID-${this.uid}`)
    if (contrastNavBar) {
